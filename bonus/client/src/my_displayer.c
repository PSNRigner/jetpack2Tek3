...
void		my_color(int id, SDL_Surface *player, SDL_Surface *ecran)
{
  char		red;
  char		green;
  char		blue;
  ...
  green = (id * 50) % 255;
  blue = (id + 100) * 99 % 255;
  SDL_FillRect(player, NULL, SDL_MapRGB(ecran->format, red, green, blue));
}
...
void		create_players(t_client *client, SDL_Surface *ecran)
{
  ...
  while  (tmp && client->map)
	...
	my_color(tmp->id, player, ecran);
      SDL_SetAlpha(player, SDL_SRCALPHA, tmp->id == client->id ? 255 : 155);
